...
export async function GET(request: NextRequest) {
  try {
    ...
    const response = await fetch(`${backendBaseUrl}/api/buildings${search ? `?${search}` : ""}`, {
      method: "GET",
      headers,
      cache: "no-store",
    });
    ...
    if (!response.ok) {
      return NextResponse.json(
        body || { message: "Failed to fetch buildings" },
...
export async function POST(request: Request) {
  try {
    ...
    const response = await fetch(`${backendBaseUrl}/api/buildings`, {
      method: "POST",
      headers,
    ...
    if (!response.ok) {
      return NextResponse.json(
        body || { message: "Failed to create building" },
        { status: response.status }
      );
    }
...
export async function PUT(request: Request) {
  try {
    ...
    const response = await fetch(`${backendBaseUrl}/api/buildings/${buildingId}`, {
      method: "PUT",
      headers,
      body: JSON.stringify(payload),
      cache: "no-store",
