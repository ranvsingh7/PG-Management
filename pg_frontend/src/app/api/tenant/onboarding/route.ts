...
export async function GET() {
  try {
    ...
    const response = await fetch(`${backendBaseUrl}/api/tenant/onboarding`, {
      method: "GET",
      headers: {
    ...
    return NextResponse.json(body || { message: "Failed to fetch onboarding status" }, { status: response.status });
  } catch {
