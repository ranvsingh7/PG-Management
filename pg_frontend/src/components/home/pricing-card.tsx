  ...
  return (
    <article
      className={`relative overflow-visible rounded-3xl border px-6 pb-6 pt-6 transition hover:-translate-y-0.5 hover:shadow-[var(--shadow-pricing-hover)] ${
        plan.highlight
          ? "border-[var(--color-emerald-hover)] [background:linear-gradient(to_bottom,var(--color-emerald-tint),var(--color-surface))] pt-9 shadow-[var(--shadow-pricing-highlight)]"
          : "border-[var(--color-border)] bg-[var(--color-surface)]"
      }`}
    >
      {plan.highlight ? (
        <span className="absolute left-1/2 top-0 -translate-x-1/2 -translate-y-1/2 rounded-full bg-[var(--color-emerald)] px-3 py-1 text-[10px] font-extrabold uppercase tracking-[0.14em] text-[var(--color-text-inverse)] shadow-[var(--shadow-badge)]">
          Most Popular
        </span>
      ) : null}

      <p className="text-sm font-extrabold uppercase tracking-[0.16em] text-[var(--color-text-secondary)]">
        {plan.name}
      </p>
