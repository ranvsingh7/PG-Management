...
export async function GET() {
  try {
    ...
    const response = await fetch(`${backendBaseUrl}/api/tenant/profile`, {
      method: "GET",
      headers,
      cache: "no-store",
    });
    ...
    return NextResponse.json(body || { message: "Failed to fetch profile" }, { status: response.status });
  } catch {
...
export async function PUT(request: Request) {
  try {
    ...
    return NextResponse.json(body || { message: "Failed to update profile" }, { status: response.status });
  } catch {
    return NextResponse.json({ message: "Backend not reachable" }, { status: 500 });
  }
}
