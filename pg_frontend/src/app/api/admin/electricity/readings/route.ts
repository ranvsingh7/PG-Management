...
export async function GET(request: NextRequest) {
  try {
    ...
    const response = await fetch(`${backendBaseUrl}/api/electricity/readings${search ? `?${search}` : ""}`, {
      method: "GET",
      headers,
      cache: "no-store",
    });
    ...
    if (!response.ok) {
      return NextResponse.json(
        body || { message: "Failed to fetch electricity readings" },
...
export async function POST(request: Request) {
  try {
    ...
    if (!response.ok) {
      return NextResponse.json(
        body || { message: "Failed to save electricity reading" },
        { status: response.status }
      );
    }
