...
export async function GET(request: NextRequest) {
  try {
    ...
    const response = await fetch(`${backendBaseUrl}/api/tenants${search ? `?${search}` : ""}`, {
      method: "GET",
      headers,
      cache: "no-store",
    });
    ...
    if (!response.ok) {
      return NextResponse.json(
        body || { message: "Failed to fetch tenants" },
...
export async function POST(request: Request) {
  try {
    ...
    const response = await fetch(`${backendBaseUrl}/api/tenants`, {
      method: "POST",
      headers,
    ...
    if (!response.ok) {
      return NextResponse.json(
        body || { message: "Failed to create tenant" },
        { status: response.status }
      );
    }
