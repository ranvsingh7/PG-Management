...
export async function GET() {
  try {
    ...
    const response = await fetch(`${backendBaseUrl}/api/tenant/documents`, {
      method: "GET",
      headers,
      cache: "no-store",
    });
    ...
    return NextResponse.json(body || { message: "Failed to fetch documents" }, { status: response.status });
  } catch {
...
export async function POST(request: Request) {
  try {
    ...
    return NextResponse.json(body || { message: "Failed to add document" }, { status: response.status });
  } catch {
    return NextResponse.json({ message: "Backend not reachable" }, { status: 500 });
  }
}
