...
export async function POST() {
  try {
    ...
    const response = await fetch(`${backendBaseUrl}/api/tenant/onboarding/complete`, {
      method: "POST",
      headers: {
    ...
    return NextResponse.json(body || { message: "Failed to complete onboarding" }, { status: response.status });
  } catch {
